...
def is_invitation_code_valid(invitation_code: str) -> bool:
    # 在你的数据库中查询邀请码是否有效
    connection = mysql.connect(host='localhost', user='root', password='200408150928', database='ssisdb')  # 请替换为你的数据库连接信息
...
def register_user(username: str, password: str) -> int:
    try:
        ...
        connection = mysql.connect(host='localhost', user='root', password='200408150928', database='ssisdb')
        cursor = connection.cursor()
